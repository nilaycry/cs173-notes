...
export default function FinalReviewPage() {
  const notes = getAllNotes("final-review");
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            A focused bridge between the full course notes and the practice final:
            topic map, proof strategies, construction patterns, and self-checks.
          </p>
        </section>
        ...
        >
          ...
          >
            <span style={labelStyle}>available guides</span>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            {notes.map((note) => (
              <Link
                key={note.slug}
                href={`/final-review/${note.slug}`}
                style={{ textDecoration: "none", color: "inherit" }}
              >
                <div className="cs-note-row" style={rowStyle}>
                  <div>
                    <span style={rowTitleStyle}>{note.title}</span>
                    <span style={rowDescriptionStyle}>{note.description}</span>
                  </div>
                  <span style={rowMetaStyle}>{typeLabel(note.noteType)}</span>
                </div>
              </Link>
            ))}
          </div>
